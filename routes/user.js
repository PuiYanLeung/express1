...
const register = async (req, res, next) => {
  ...
  req.user.email? res.status(201).send({ msg: "registered successfully", user_id: req.user.id, user_email: req.user.email }) : res.status(401).send({ msg: "User already exists" }); 
